...
class RainTPL{
	...
	protected function check_template( $tpl_name ){
		...
		if( !isset($this->tpl['checked']) ){
			...
			$this->tpl['tpl_filename']          = $tpl_dir . $tpl_basename;
			!empty(self::$tpl_ext)?$this->tpl['filename']. '.' . self::$tpl_ext:NULL;	// template filename
	...
	*/
	...
	protected function compileFile( $tpl_basename, $tpl_basedir, $tpl_filename, $cache_dir, $compiled_filename ){
		...
		//read template file
		empty($this->tpl['source'])?$this->tpl['source'] = $template_code = file_get_contents( $tpl_filename ):NULL;
